            ...
            if AutoEng_Config['engagementnmaptcp'] == "Complete" and AutoEng_Config['engagementtestssl'] == "Awaiting TCP Scan Results.": #Check if nmap TCP scan is complete and run testssl if required
                ...
                if os.stat(TCPFileCheck).st_size != 0:
                    if os.stat(CurrEngFolder+"/data/tls_hosts.txt").st_size == 0:
                        ...
                        SSLCommand = "sed -i 's/engagementtestssl = Awaiting TCP Scan Results./engagementtestssl = Running/' /root/Documents/data/emmett_config.ini && cd /root/Documents/output/tls/raw_outputs && script -q -c 'testssl --warnings=batch --log --json --file ../../../data/tls_hosts.txt' /root/Documents/output/tls/"+SSLHTMLFileName+" && /root/shared/EzModeSSL -d . -o ../"+Eng_Config['client']+" && sed -i 's/engagementtestssl = Running/engagementtestssl = Complete/' /root/Documents/data/emmett_config.ini"
                        globals()['EngagementTestssl'] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",SSLCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name="Engagementtestssl", labels=["Emmett","Engagement"], volumes=[DeloreanBuildVolume, DocumentsVolume])
                else:
