...
with open(ConfigFileLocation, 'w') as configfile:
    config_object.write(configfile)
TCPNmapCommand = "sed -i 's/engagementnmaptcp = Not Started/engagementnmaptcp = Running/' /root/Documents/data/emmett_config.ini && nmap -sV -p- -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/auto_tcp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/auto_tcp_raw.txt -o /root/Documents/output/nmap/"+TCPNmapFileName+" && sed -i 's/engagementnmaptcp = Running/engagementnmaptcp = Complete/' /root/Documents/data/emmett_config.ini"                   
globals()['EngagementNmapTCP'] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",TCPNmapCommand], network_mode="container:Emmett", cap_add=["NET_ADMIN"], privileged=False, name="Engagementnmaptcp", labels=["Emmett","Engagement"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
UDPNmapFileName = "udp_output_"+now.strftime("%d%m%Y_%H%M")
UDPNmapCommand = "sed -i 's/engagementnmapudp = Not Started/engagementnmapudp = Running/' /root/Documents/data/emmett_config.ini && nmap -sU -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/auto_udp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/auto_udp_raw.txt -o /root/Documents/output/nmap/"+UDPNmapFileName+" && sed -i 's/engagementnmapudp = Running/engagementnmapudp = Complete/' /root/Documents/data/emmett_config.ini"
globals()['EngagementNmapUDP'] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash", "-c", UDPNmapCommand], network_mode="container:Emmett", cap_add=["NET_ADMIN"], privileged=False, name="Engagementnmapudp", labels=["Emmett", "Engagement"], volumes=[DocumentsVolume, DeLoreansBuildVolume])                      
globals()['Livehosts'] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/sh", "-c", "sed -i 's/engagementlivehosts = Not Started/engagementlivehosts = Running/' /root/Documents/data/emmett_config.ini && nmap -p- --open -iL /root/Documents/data/hosts.txt -oG - | grep '/open' | awk '{ print $2 }' > /root/Documents/output/nmap/auto_livehosts.txt && sed -i 's/engagementlivehosts = Running/engagementlivehosts = Complete/' /root/Documents/data/emmett_config.ini"], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name="Engagementlivehosts", labels=["Emmett", "Engagement"], volumes=[DocumentsVolume])