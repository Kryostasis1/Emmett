  ...
  TOOL LIST:
    ...
    wpscan [Optional Number of hosts per scan (Default 5)]
							...
							with open(HTTPHostFileCheck, 'r') as file:
								...
								if len(HTTPHostRead) > DirbMaxHostNum:
									...
									for i in DirbSubList:
										...
										DirbCommand = "mv /root/Documents/data/"+DirbNewName+"_hosts.txt /tmp/ && python /root/shared/tools/dirbloop.py -f /tmp/"+DirbNewName+"_hosts.txt -o /root/Documents/output/http/"+DirbFileName
										globals()['DirbContainer%s' % DirbCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c", DirbCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=DirbNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
									...
									DirbCommand = "python /root/shared/tools/dirbloop.py -f /root/Documents/data/http_hosts.txt -o /root/Documents/output/http/"+DirbFileName
									globals()['DirbContainer%s' % DirbCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",DirbCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=DirbNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
					...
					if re.search('livehosts', MasterInput[1], re.IGNORECASE):
						...
						globals()['LivehostsContainer%s' % LivehostsCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c", "nmap -p- --open -iL /root/Documents/data/hosts.txt -oG - | grep '/open' | awk '{ print $2 }' > /root/Documents/output/nmap/livehosts.txt"], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=NewLivehostsName, labels=["Emmett"], volumes=[DocumentsVolume])
							...
							with open(HTTPHostFileCheck, 'r') as file:
								...
								if len(HTTPHostRead) > NiktoMaxHostNum:
									...
									for i in NiktoSubList:
										...
										NiktoCommand = "mv /root/Documents/data/"+NiktoNewName+"_hosts.txt /tmp/ && nikto -h /tmp/"+NiktoNewName+"_hosts.txt |& tee /root/Documents/output/http/"+NiktoFileName
										globals()['NiktoContainer%s' % NiktoCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c", NiktoCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=NiktoNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
									...
									NiktoNewName = 'Nikto%s' % NiktoCounter
									NiktoFileName = NiktoNewName+"_output_"+now.strftime("%d%m%y_%H%M")+".txt"
									NiktoCommand = "nikto -h /root/Documents/data/http_hosts.txt >> /root/Documents/output/http/"+NiktoFileName
									globals()['NiktoContainer%s' % NiktoCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",NiktoCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=NiktoNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
...
All outputs saved to current client folder under ./output/nmap/ directory.
					...
					if re.search('nmap', MasterInput[1], re.IGNORECASE) and len(MasterInput) == 3:
						if re.search('tcp', MasterInput[2], re.IGNORECASE):
							...
							TCPNmapCommand = "nmap -sV -p- -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/tcp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/tcp_raw.txt -o /root/Documents/output/nmap/"+TCPNmapFileName
							globals()['TCPNmapContainer%s' % TCPNmapCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",TCPNmapCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=TCPNewNmapName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
						...
						if re.search('udp', MasterInput[2], re.IGNORECASE):
							...
							UDPNmapCommand = "nmap -sU -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/udp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/udp_raw.txt -o /root/Documents/output/nmap/udp_output"+UDPNmapFileName
							globals()['UDPNmapContainer%s' % UDPNmapCounter] = client.containers.run("delorean", entrypoint=["/bin/bash","-c",UDPNmapCommand], detach=True, remove=True, cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=UDPNewNmapName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
					...
					if re.search('nmap', MasterInput[1], re.IGNORECASE) and len(MasterInput) == 4:
						if re.search('tcp', MasterInput[2]) and re.search('no-ping', MasterInput[3]):
							...
							TCPNewNmapName = 'Nmaptcp%s' % TCPNmapCounter
							TCPNmapFileName = "tcp_output_"+now.strftime("%d%m%y_%H%M")
							TCPNmapCommand = "nmap -sV -Pn -p- -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/tcp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/tcp_raw.txt -o /root/Documents/output/nmap/"+TCPNmapFileName
							globals()['TCPNmapContainer%s' % TCPNmapCounter] = client.containers.run("delorean", entrypoint=["/bin/bash","-c",TCPNmapCommand], detach=True, remove=True, cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=TCPNewNmapName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
					...
					if re.search('nmap', MasterInput[1], re.IGNORECASE) and len(MasterInput) == 4:
						if re.search('udp', MasterInput[2]) and re.search('no-ping', MasterInput[3]):
							...
							UDPNmapFileName = "udp_output_"+now.strftime("%d%m%y_%H%M")
							UDPNewNmapName = 'Nmapudp%s' % UDPNmapCounter
							UDPNmapCommand = "nmap -sU -Pn -iL /root/Documents/data/hosts.txt -v -oN /root/Documents/output/nmap/raw_outputs/udp_raw.txt && python /root/shared/tools/parzival.py -f /root/Documents/output/nmap/raw_outputs/udp_raw.txt -o /root/Documents/output/nmap/udp_output"+UDPNmapFileName
							globals()['UDPNmapContainer%s' % UDPNmapCounter] = client.containers.run("delorean", entrypoint=["/bin/bash","-c",UDPNmapCommand], detach=True, remove=True, cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=UDPNewNmapName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
	...
	Usage: run parzival [input filename] [output filename (optional)]
						...
						if len(MasterInput) == 3:
							...
							ParzivalCommand = "cd /root/Documents/output/nmap && python /root/shared/tools/parzival.py -f "+ParzivalInputFilename
							globals()['ParzivalContainer%s' % ParzivalCounter] = client.containers.run("delorean", detach=True, cap_add=["NET_ADMIN"], remove=True, entrypoint=["/bin/bash","-c",ParzivalCommand], network_mode="container:Emmett", privileged=False, name=NewParzivalName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
						...
						if len(MasterInput) == 4:
							...
							ParzivalCounter = ParzivalCounter+1
							NewParzivalName = 'Parzival%s' % ParzivalCounter
							ParzivalCommand = "cd /root/Documents/output/nmap && python /root/shared/tools/parzival.py -f "+ParzivalInputFilename+ParzivalOutputFilename
							globals()['ParzivalContainer%s' % ParzivalCounter] = client.containers.run("delorean", detach=True, entrypoint=["/bin/bash","-c",ParzivalCommand], cap_add=["NET_ADMIN"], remove=True, network_mode="container:Emmett", privileged=False, name=NewParzivalName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
					...
					if re.search('testssl', MasterInput[1], re.IGNORECASE):
						...
						if TLSHostFileCheck.is_file():
							SSLCommand = "cd /root/Documents/output/tls/raw_outputs && script -q -c 'testssl --warnings=batch --log --json --file ../../../data/tls_hosts.txt' /root/Documents/output/tls/"+SSLHTMLFileName+" && /root/shared/EzModeSSL -d . -o ../"+ClientName
							globals()['SSLContainer%s' % SSLCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",SSLCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=NewSSLName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
							...
							SSLCommand = "cd /root/Documents/output/tls/raw_outputs && testssl --warnings=batch --log --json --file ../../../data/hosts.txt |& tee -a /tmp/"+SSLHTMLFileName+" && cat /tmp"+SSLHTMLFileName+" | ansi2html > /root/Documents/output/tls/"+SSLHTMLFileName+" && /root/shared/EzModeSSL -d . -o ../"+ClientName
							globals()['SSLContainer%s' % SSLCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c", SSLCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=NewSSLName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
							print("\nTestSSL command \"{}\" has been executed in container ".format(colored("testssl --warnings=batch --log --json --file ../../../data/tls_hosts.txt", "yellow"))+"{}".format(colored(NewSSLName, "yellow"))+". When complete outputs can be found within {}.".format(colored("output/tls/", "yellow")))						
							...
							with open(HTTPHostFileCheck, 'r') as file:
								...
								if len(HTTPHostRead) > WpscanMaxHostNum:
									...
									for i in WpscanSubList:
										...
										WpscanCommand = "mv /root/Documents/data/"+WpscanNewName+"_hosts.txt /tmp/ && python /root/shared/tools/wpscanloop.py -f /tmp/"+WpscanNewName+"_hosts.txt -o /root/Documents/output/http/"+WpscanFileName
										globals()['WpscanContainer%s' % WpscanCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",WpscanCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=WpscanNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
									...
									WpscanNewName = 'Wpscan%s' % WpscanCounter
									WpscanFileName = WpscanNewName+"_output_"+now.strftime("%d%m%y_%H%M")+".txt"
									WpscanCommand = "python /root/shared/tools/wpscanloop.py -f /root/Documents/data/http_hosts.txt -o /root/Documents/output/http/"+WpscanFileName
									globals()['WpscanContainer%s' % WpscanCounter] = client.containers.run("delorean", detach=True, remove=True, entrypoint=["/bin/bash","-c",WpscanCommand], cap_add=["NET_ADMIN"], network_mode="container:Emmett", privileged=False, name=WpscanNewName, labels=["Emmett"], volumes=[DocumentsVolume, DeLoreansBuildVolume])
